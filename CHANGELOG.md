...
- **Переключатель «светлая/тёмная» в редакторе «Оформление»** ([`SettingsWindow.Schemes.cs`](Configuration%20Management/Views/SettingsWindow.Schemes.cs)): над списком цветов выбирается редактируемая палитра, что позволяет настроить светлую и тёмную темы одной схемы отдельно.
...
- **Версия поднята до `0.3.5.93` → `0.4.0.0`** во всех четырёх полях `<Version>`, `<AssemblyVersion>`, `<FileVersion>`, `<InformationalVersion>` в [`Configuration Management.csproj`](Configuration%20Management/Configuration%20Management.csproj).
