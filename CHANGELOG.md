...
При открытии спонсорской картинки «О программе» (`donat.png`) в полном размере в отдельном окне убрано колесо прокрутки — картинка теперь масштабируется пропорционально и целиком помещается в окне, а размер окна рассчитывается от рабочей области экрана с учётом его разрешения и масштаба (DPI).

### Изменено

- **Спонсорская картинка «О программе» в полном размере открывается без прокрутки** ([`Views/SettingsWindow.Platforms.cs`](Configuration%20Management/Views/SettingsWindow.Platforms.cs)): при клике на картинку `donat.png` она открывается в отдельном окне и масштабируется пропорционально (`Stretch=Uniform`), целиком помещаясь в окне; размер окна рассчитывается от рабочей области экрана с учётом его разрешения и масштаба (DPI, через `VisualTreeHelper.GetDpi` + `System.Windows.Forms.Screen.WorkingArea`), поэтому колесо прокрутки больше не нужно.
