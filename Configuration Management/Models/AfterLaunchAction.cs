...
public enum AfterLaunchAction
{
    ...
    None,
    /// <summary>Просто свернуть главное окно (остаётся в панели задач, не в трей).</summary>
    Minimize,
...
public static class AfterLaunchActionHelper
{
    ...
    public static string ToSettingString(this AfterLaunchAction action)
        => action switch
        {
            AfterLaunchAction.Minimize => "Minimize",
    ...
    public static AfterLaunchAction Parse(string? value)
    {
        ...
        if (value.Trim().Equals("Minimize", StringComparison.OrdinalIgnoreCase))
            return AfterLaunchAction.Minimize;
